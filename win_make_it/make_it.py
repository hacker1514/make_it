...
def print_help():
    console.print(Panel(
        ...
        "[cyan]API Key:[/cyan]\n"
        "  Stored permanently in your system environment as [yellow]AI_API_KEY[/yellow].\n"
        "  On first run, you will be prompted to enter it once.",
...
def _save_env_windows(key: str, value: str) -> bool:
    try:
        import winreg
...
def _save_env_unix(key: str, value: str) -> bool:
    try:
        shell = os.environ.get("SHELL", "")
        if "zsh" in shell:
            rc = os.path.join(HOME_DIR, ".zshrc")
        elif "fish" in shell:
            rc = os.path.join(HOME_DIR, ".config", "fish", "config.fish")
        else:
            rc = os.path.join(HOME_DIR, ".bashrc")
        export_line = f'\nexport {key}="{value}"\n'
...
def _key_file() -> str:
    return os.path.join(HOME_DIR, ".make_it", ".key")
...
def _persist_key(value: str):
    os.makedirs(os.path.join(HOME_DIR, ".make_it"), exist_ok=True)
    try:
        ...
        pass
    if sys.platform == "win32":
        _save_env_windows(config.ENV_VAR_NAME, value)
    else:
        _save_env_unix(config.ENV_VAR_NAME, value)


def setup_api_key() -> str:
    key = os.environ.get(config.ENV_VAR_NAME, "").strip()
    if not key:
        key = _load_saved_key()
    if key:
        os.environ[config.ENV_VAR_NAME] = key
        config.API_KEY = key
        return key

    console.print()
    console.print(Panel(
        f"[bold yellow]API key not found.[/bold yellow]\n\n"
        f"Enter your key below.\n"
        f"It will be saved permanently — [bold]you won't be asked again[/bold].",
    ...
    while True:
        try:
            entered = input("  Paste your API key here: ").strip()
        ...
        if len(entered) < 10:
            console.print("  [red]That looks too short. Please paste the full key.[/red]")
            continue

        os.environ[config.ENV_VAR_NAME] = entered
        config.API_KEY = entered
        _persist_key(entered)

        console.print(
            f"\n  [green]✓ Saved.[/green] "
            f"[dim]You won't be asked again.[/dim]\n"
        )
        return entered
